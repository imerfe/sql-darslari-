create database homework_7_sql 
use homework_7_sql
...
select departmentname, count( EmployeeID) as employees_counted_numbers from employees group by departmentname 


--Write a query to show the minimum and maximum Price of products grouped by Category. Use products table.\

select category, min(price) as new_column_min, max(price) as new_column_max from Products group by category 


--Write a query to calculate the total (SUM) sales per Customer in the Sales table.

select customerid, sum(saleamount) as new_sales from sales group by customerid 
...
SELECT departmentname, count(employeeid) as counted 
FROM employees 
...
select ProductID, sum(saleamount) as total_sale, avg(saleamount) avg_sale from sales group by productid


--Write a query that uses COUNT(columnname) to count the number of employees from the Department HR.

select departmentname, count(FirstName) as new_counting from employees 
group by departmentname
...
select departmentname, max(salary) as highest,  min(salary) as lowest  from employees group by departmentname
...
select departmentname, avg(salary) avg_salary from Employees group by  departmentname
...
select departmentname, avg(salary) as avg_salary, count(firstname) as counted from employees group by  departmentname

--Write a query that uses HAVING to filter product categories with an average price greater than 400.

select category, avg(price) as fixed_price 
from products
group by category 
...
select year(SaleDate) as year_saling, sum(saleamount) as saling_amount from sales
...
select departmentname, sum(salary) as counted from employees 
group by departmentname
...
select p.category, avg(s.saleamount) as new_column 
from sales s 
...
select c.CustomerID, sum(s.saleamount) as new_column 
from sales s
...
select departmentname, sum(salary) as salary_sum_new, avg(salary) as salary_avg_new  from employees 
group by departmentname 
...
select c.customerid, max(o.TotalAmount ) as max_q, min(o.TotalAmount ) as min_q 
from Orders o 
join  customers c on c.customerID=o.CustomerID
group by c.customerid
having max(o.TotalAmount )<50 and 
min(o.TotalAmount )<50
...
select  month(s.SaleDate) as saling_period, sum(s.saleamount) as all_sale, count(distinct p.productname) as motnhly
from sales s
join products p on p.ProductID=s.ProductID 
group by month(s.SaleDate)
having count(distinct p.ProductName)>8


--Write a query to find the MIN and MAX order quantity per Year. From orders table. (Do some research)

select YEAR(ORDERDATE) AS YEARS, min(QUANTITY) AS MIN_QUANTITY, MAX(QUANTITY) AS MAX_QUANTITY FROM ORDERS
