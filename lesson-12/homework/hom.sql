create database homework_12_sql 
use homework_12_sql 
...
select e.name as name_of_employee,e.managerid, e.salary as employee_salary, e2.salary as manager_salary from employee e 
...
select  email as emails  from person
group by email
...
Find total Sales amount for the orders which weights more than 50 
for each customer along with their least weight.(from TSQL2012 database, Sales.Orders Table)
...
select isnull(c1.item,' ') as item_cart_1,isnull(c2.item,' ') as item_cart_2 from cart1 c1 
full  join cart2 c2 on c1.item=c2.item
order by  item_cart_1 desc
...
/*SELECT 
    ...
    Score,
    CASE
        WHEN CAST(SUBSTRING_INDEX(Score, ':', 1) AS SIGNED) > CAST(SUBSTRING_INDEX(Score, ':', -1) AS SIGNED) 
            THEN SUBSTRING_INDEX(Match, '-', 1)
        WHEN CAST(SUBSTRING_INDEX(Score, ':', 1) AS SIGNED) < CAST(SUBSTRING_INDEX(Score, ':', -1) AS SIGNED) 
            THEN SUBSTRING_INDEX(Match, '-', -1)
        ELSE 'Draw'
    END AS Result
