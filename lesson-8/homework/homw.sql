...
select FirstName from Customers where city like 'L%'
...
select StockQuantity, iif(StockQuantity < 30,'Low Stock','Sufficient') as new from Products 

--Using Customers table, find the total number of customers in each country.

select count(*) as counted_names,country from Customers group by Country

--Using Orders table, find the minimum and maximum quantity ordered.

select min(quantity) as minumum, max(quantity) as maximum from orders 
...
SELECT AVG(QUANTITY) AS NEW_AVERAGE, YEAR(OrderDate) FROM ORDERS GROUP BY YEAR(OrderDate) 
...
SELECT PRICE, CASE WHEN PRICE <100 THEN 'LOW'
WHEN 100 < PRICE AND PRICE < 500 THEN 'MID'
...
SELECT SUM(SaleAmount) AS TOTAL_SALES, PRODUCTID AS THEIR_ICON FROM SALES GROUP BY ProductID 
...
SELECT TOP 3 CustomerID, SUM(TotalAmount) AS TOTALS FROM  INVOICES 
...
SELECT P.PRODUCTNAME, COUNT(S.SaleID) FROM SALES S
