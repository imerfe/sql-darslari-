create database homework_11_sql_with_new_tables 
use homework_11_sql_with_new_tables 
...
select o.OrderID, c.firstname as CustomerName, year(o.OrderDate) as orderdate 
from orders o 
...
SELECT d.DepartmentName, e.Name AS TopEmployeeName, e.Salary
FROM Employees e
INNER JOIN (
    SELECT DepartmentID, MAX(Salary) AS MaxSalary
    FROM Employees
    GROUP BY DepartmentID
) AS dept_max ON e.DepartmentID = dept_max.DepartmentID AND e.Salary = dept_max.MaxSalary
...
select C.FIRSTNAME AS CustomerName,C.COUNTRY, O.OrderID , YEAR(O.OrderDate) FROM Customers C
INNER JOIN ORDERS O ON O.CUSTOMERID=C.CUSTOMERID
WHERE C.COUNTRY='USA' AND YEAR(ORDERDATE)=2023 
...
SELECT C.FIRSTNAME AS CustomerName, NEW_TABLE_DERIVED.total_orders FROM CUSTOMERS C
INNER JOIN (
SELECT CUSTOMERID, COUNT(ORDERID)  as total_orders FROM ORDERS 
GROUP BY CUSTOMERID 
) AS NEW_TABLE_DERIVED on NEW_TABLE_DERIVED.CustomerID=c.CustomerID 
...
/*select  c.firstname as  CustomerName,   NEW_DERIVED_TABLES.ORDERED_RECENTS
FROM Customers C 
INNER JOIN (
SELECT CUSTOMERID, YEAR(ORDERDATE) AS ORDERED_RECENTS FROM ORDERS 
ORDER BY YEAR(ORDERDATE) DESC 
) AS NEW_DERIVED_TABLES ON NEW_DERIVED_TABLES.CUSTOMERID=C.CUSTOMERID*/
...
LEFT JOIN (
    ...
    INNER JOIN (
        ...
        GROUP BY 
            CustomerID )   
			O2 ON O1.CustomerID = O2.CustomerID AND O1.OrderDate = O2.MaxOrderDate  )
...
select  c.firstname as CustomerName, o.OrderID, o.totalamount as OrderTotal from customers c 
...
select p.ProductName, year(s.SaleDate) as sales_date, s.SaleAmount from products p 
...
select p.ProductName, year(s.SaleDate) as year_of_sale, p.StockQuantity from Products p 
inner join sales s on p.ProductID=s.ProductID 
...
select e.name as EmployeeName, d.DepartmentName, year(HireDate) AS HIRED_YEAR from Employees e 
inner join departments d on d.DepartmentID=e.DepartmentID 
where d.DepartmentName='sales' and year(HireDate)>2020
...
SELECT  C.FIRSTNAME AS CustomerName, O.OrderID, C.Address,C.COUNTRY, O.OrderDate FROM ORDERS O 
INNER JOIN CUSTOMERS C ON C.CUSTOMERID=O.CUSTOMERID  
WHERE C.Address LIKE '4%' AND C.COUNTRY='USA'
...
ON NEW_TABLE.Category = c.CategoryID;
...
select c.firstname as CustomerName, c.City, o.OrderID, o.totalAmount from Customers c
inner join orders o on c.customerid=o.CustomerID 
...
inner join departments d on d.DepartmentID=e.DepartmentID
WHERE 
d.DepartmentName IN ('HR','FINANCE') AND e.name LIKE '%A%' AND e.name LIKE '%E%' AND e.name LIKE '%O%'AND e.name LIKE '%U%'
...
SELECT P.ProductName, S.SALEAMOUNT AS QuantitySold, P.Price FROM PRODUCTS P 
INNER JOIN SALES S  ON S.PRODUCTID=P.PRODUCTID
WHERE S.SALEAMOUNT> 500 AND P.PRICE>100
