create database homework_10_sql 
use homework_10_sql 
...
select e.name,d.departmentname, e.Salary from employees e inner join Departments d on d.DepartmentID=e.DepartmentID
...
select c.FirstName, c.LastName, year(o.OrderDate) as orderdate from customers c 
...
select e.Name, isnull(d.DepartmentName, 'hech qaysi departmentda ishlamaydi') 
from employees e 
...
select s.SupplierName, ISNULL (p.ProductName, 'ORDER IS NOT AVAILABLE ') AS ProductName from Suppliers s 
...
--Using the Employees table, write a query to show each employee's name along with the name of their manager.
 ...
 select  M.Name AS MANAGERNAME, e.Name as employeename FROM EMPLOYEES M
left JOIN EMPLOYEES e ON E.EmployeeID=m.ManagerID 
...
SELECT  C.CustomerID,C.FirstName, C.LastName, SUM(O.Quantity) FROM CUSTOMERS C
INNER JOIN Orders O ON C.CustomerID=o.CustomerID
...
SELECT D.DepartmentName, COUNT(E.EmployeeID) AS EMPLOYEECOUNTED FROM Departments D 
...
SELECT  P.ProductID, P.ProductName,S.SaleID   FROM    SALES S
...
SELECT c.customerid, C.FirstName, C.LastName, sum(quantity)  AS TotalQuantity FROM Customers C
INNER JOIN ORDERS o on c.customerid=o.CustomerID 
group by c.customerid, C.FirstName, C.LastName
...
SELECT 
    E1.Name AS Employee_1_Name, 
    E2.Name AS Employee_2_Name, 
    E1.ManagerID
FROM 
    Employees E1
INNER JOIN 
...
SELECT O.OrderID,C.FirstName, C.LastName, YEAR(O.OrderDate) AS YEAR_OF_ORDERING  FROM ORDERS O 
...
INNER JOIN DEPARTMENTS D ON E.DepartmentID=D.DepartmentID
WHERE DepartmentName='SALES' AND E.SALARY>60000
...
select  E1.EMPLOYEEID, E1.Name AS EMPLOYEES_NAME_1 , AVG(E1.Salary) AS EMPLOYEES_SALARY_1 ,E2.Salary AS EMPLOYEES_SALARY_2  FROM EMPLOYEES E1
INNER JOIN EMPLOYEES E2 ON E1.EmployeeID=E2.EmployeeID
GROUP BY E1.EMPLOYEEID, E1.Name
HAVING AVG(E1.Salary)>E2.Salary
...
select o.OrderID, year(o.OrderDate) as orderdate from orders o 
left join payments p on o.orderid=p.orderid
where year(o.orderdate)>2020 and paymentid is null 
...
E1.Salary 
FROM Employees e1
inner join Employees e2 on e2.managerid=e1.managerid
AND e1.EmployeeID <e2.EmployeeID
...
select e.Name as EmployeeName, d.DepartmentName as departmentname from Employees E 
INNER JOIN Departments D ON D.DepartmentID=E.DepartmentID
...
select S.StudentID AS STUDENDTID,c.CourseName, S.Name AS StudentName from Students S 
INNER JOIN ENROLLMENTS E ON E.StudentID=S.StudentID 
