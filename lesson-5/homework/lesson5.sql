...
create database homework_5_sql
...
CREATE TABLE Customers ( CustomerID INT PRIMARY KEY, FirstName VARCHAR(100), LastName VARCHAR(100), Email VARCHAR(100), Phone VARCHAR(50), Address VARCHAR(255), City VARCHAR(100), State VARCHAR(100), PostalCode VARCHAR(20), Country VARCHAR(100) );
 ...
 select CustomerID, FirstName, LastName, Email, Phone, Address, City, State, PostalCode, Country as client from customers
...
SELECT DISTINCT FirstName , address from customers 
...
select PRICE, IIF(PRICE>1000,'Expensive','Affordable') AS DECISION FROM PRODUCTS
...
SELECT  SaleID, ProductID, CustomerID, CASE WHEN SaleAmount> 500 THEN 'Top Tier'
WHEN SaleAmount BETWEEN 200 AND 500 THEN 'Mid Tier'
ELSE 'Low Tier'END AS BINGO FROM SALES 
