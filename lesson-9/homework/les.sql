...
SELECT O.ORDERID,o.OrderDate, P.PRICE FROM ORDERS O
INNER JOIN PRODUCTS P ON 
P.ProductID=O.ProductID WHERE P.PRICE>100
...
SELECT O.Quantity ,P.ProductName, O.ORDERDATE, P.StockQuantity FROM ORDERS O 
...
inner join customers c on c.customerid=s.CustomerID where S.SaleAmount=500 or s.saleamount>500 
...
--Using Products, Suppliers table List product and supplier names where supplier name contains “Tech”.
	...
	SELECT P.ProductName, S.SupplierName, S.SUPPLIERID FROM SUPPLIERS S 
	INNER JOIN PRODUCTS P ON S.SUPPLIERID=P.SUPPLIERID WHERE S.SupplierName LIKE '%TECH%'

--Using Orders, Payments table Show orders where payment amount is less than total amount.
	
	SELECT P.Amount, O.TotalAmount FROM ORDERS O INNER JOIN PAYMENTS P ON  O.ORDERID=P.ORDERID  WHERE O.TotalAmount > P.Amount

---Using Employees table List employee names with salaries greater than their manager’s salary.

SELECT E1.NAME AS EMPLOYEE_NAME, E1.SALARY AS EMPLOYEE_SALARY, M.NAME AS MANADGER_NAME,M.SALARY AS MANAGER_SALARY FROM EMPLOYEES E1
INNER JOIN  EMPLOYEES M ON E1.EmployeeID=M.ManagerID 
WHERE M.SALARY<E1.SALARY

--Using Products, Categories table Show products where category is either 'Electronics' or 'Furniture'.

 SELECT P.PRODUCTNAME,C.CategoryName FROM PRODUCTS P 
 INNER JOIN CATEGORIES C ON CategoryID=ProductID
 WHERE C.CategoryName IN ('Electronics', 'Furniture')

--Using Sales, Customers table Show all sales from customers who are from 'USA'.

SELECT S.SaleID, C.FirstName, C.COUNTRY FROM Customers C INNER JOIN SALES S 
ON C.CustomerID=S.CustomerID WHERE C.COUNTRY='USA'


--Using Orders, Customers table List orders made by customers from 'Germany' and order total > 100.


SELECT C.FIRSTNAME, O.ORDERID,C.COUNTRY, O.TOTALAMOUNT FROM Customers C 
INNER JOIN ORDERS O ON O.CustomerID=C.CustomerID 
WHERE C.COUNTRY='GERMAN' AND O.TOTALAMOUNT>100
...
select p.ProductName,o.OrderID, pay.Amount, o.Quantity,p.Price from payments pay
...
E2.NAME AS THEIR_BOSS_NAME, 
E1.SALARY AS EMPLOYEE_SALARY, 
E2.SALARY AS THEIR_BOSSES_SALARY
FROM EMPLOYEES E2
INNER JOIN EMPLOYEES E1 ON E1.EmployeeID=E2.ManagerID
...
SELECT O.OrderID, C.FirstName,C.CUSTOMERID  FROM ORDERS O
LEFT JOIN PAYMENTS P ON P.ORDERID=O.CustomerID
